...
from games.pente.constants import (PLAYER_BLACK,PLAYER_WHITE,FIRST_PLAYER,GAME_NOT_STARTED,GAME_RUNNING,GAME_DRAW,GAME_OVER,GAME_MODE_HUMAN_VS_HUMAN,GAME_MODE_HUMAN_VS_AI,)
...
class PenteGame(GameInterface):
    ...
    def get_state(self):
        return {"board": self.board.get_board_state(),"current_player": self.current_player,"winner": self.result.winner,"game_state": self.game_state,"game_over": self.result.game_over,"draw": self.result.draw,"move_count": self.move_count,"capture_counts": dict(self.capture_counts),}
    def get_capture_count(self, player):
        return self.capture_counts.get(player,0,)
    def get_capture_counts(self):
        return dict(self.capture_counts)
