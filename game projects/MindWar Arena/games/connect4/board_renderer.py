...
class BoardRenderer:
    ...
    def render(self, board, game_result=None):
        ...
        for row in range(BOARD_ROWS):
            for column in range(BOARD_COLUMNS):
                ...
                if value == PLAYER_RED:
                    self.draw_disc(row, column, PLAYER_RED_COLOR)

                elif value == PLAYER_YELLOW:
                    self.draw_disc(row, column, PLAYER_YELLOW_COLOR)
    ...
    def draw_disc(self, row, column, color):
    ...
    def screen_to_column(self, x, y):
        ...
        return int((x - self.origin_x) // CELL_SIZE)
