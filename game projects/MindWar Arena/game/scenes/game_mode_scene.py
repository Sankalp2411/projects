...
class GameModeScene(Scene):
    ...
    def __init__(self, game_name):
        ...
        self.modes = [
            ("Human vs Human", GAME_MODE_HUMAN_VS_HUMAN),
            ("Human vs AI", GAME_MODE_HUMAN_VS_AI),
        ]
    ...
    def enter(self):
        super().enter()
        Logger.info(
            f"[GameModeScene] Choosing mode for {self.game_name}"
        )
    ...
    def update(self):
        ...
        if Input.is_key_clicked(pygame.K_UP):
            ...
            Logger.info(
                f"[GameModeScene] Mode: {mode_name}"
            )

            game_class = GameRegistry.get_game_class(
                self.game_name
            )
